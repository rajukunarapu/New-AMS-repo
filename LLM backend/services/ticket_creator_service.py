...
class TicketSessionManager:
    """Thread-safe in-memory session store for active ticket creation drafts with 30-minute auto-expiry."""
    ...
    def _cleanup_expired(cls):
        """Remove sessions older than SESSION_TIMEOUT_SECONDS (called within lock)."""
        import time
        now = time.time()
        expired_keys = [k for k, ts in cls._timestamps.items() if (now - ts) > cls.SESSION_TIMEOUT_SECONDS]
        for k in expired_keys:
            cls._sessions.pop(k, None)
            cls._timestamps.pop(k, None)
            print(f"[TicketSessionManager] Expired inactive session: {k}")
    ...
    def get_draft(cls, key: str) -> Optional[Dict[str, Any]]:
        with cls._lock:
            ...
            if key in cls._sessions:
                cls._timestamps[key] = time.time()  # Refresh on access
...
async def classify_user_intent_with_llm(prompt: str, history: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    """
    ...
    try:
        raw_res = await _call_llm(user_payload, system_instruction=system_instruction, json_response=True)
        if raw_res:
...
async def is_ticket_creation_prompt(prompt: str, history: Optional[List[Dict[str, Any]]] = None) -> bool:
    """
    ...
    res = await classify_user_intent_with_llm(prompt, history=history)
...
async def is_explicit_query_prompt(prompt: str, history: Optional[List[Dict[str, Any]]] = None) -> bool:
    """
...
async def extract_fields_with_llm(prompt: str, current_draft: Dict[str, Any], known_clients: List[str]) -> Dict[str, Any]:
    """
...
"""
    ...
    raw_response = await _call_llm(extraction_prompt, system_instruction=system_prompt, json_response=True)
...
async def finalize_draft_fields(draft: Dict[str, Any], user_email: str = "", known_clients: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    if not res.get("assigntogroup") and res.get("descriptionofTicket"):
        try:
            auto_group = await assign_group(res["descriptionofTicket"])
            if auto_group:
...
async def analyze_user_intent_on_draft(
    user_message: str,
    ...
    if missing_fields and pending_field and not await is_explicit_query_prompt(user_message):
    ...
    if missing_fields and is_explicit_query_prompt_keyword(user_message):
        return "unrelated", {}
...
"""
    ...
    raw_response = await _call_llm(intent_prompt, system_instruction=system_instruction, json_response=True)
    if raw_response:
        try:
            cleaned = raw_response.strip()
            if cleaned.startswith("```"):
            ...
            elif intent == "unrelated" and missing_fields and not is_explicit_query_prompt_keyword(user_message):
    ...
    if missing_fields and pending_field and not is_explicit_query_prompt_keyword(user_message):
        ...
        if is_missing or not val:
            ...
            if key in ["reportedon", "reportedontime"]:
                ...
                if val_str.startswith("data:image/"):
                    ...
                    if ";base64," in clean_src:
                        parts = clean_src.split(";base64,", 1)
                        clean_src = f"{parts[0]};base64,{re.sub(r'\s+', '', parts[1])}"
...
async def submit_ticket_to_ams(draft: Dict[str, Any], ams: AMSApi) -> Dict[str, Any]:
    """
    Calls AMS live API /api/Ticket/CreateTicket with the finalized payload (async).
    """
    payload = {
